...
{
	switch (system_mode) {
	case 0: return AC_MODE_OFF;       /* Off */
	case 3: return AC_MODE_COOLING;   /* Cool */
	case 4: return AC_MODE_HEATING;   /* Heat */
	case 7: return AC_MODE_VENTILATION; /* FanOnly */
	default: return AC_MODE_COOLING;
	}
}
...
{
	switch (fan_mode) {
	case 0: return FAN_SPEED_1;      /* Off → lowest */
	case 1: return FAN_SPEED_1;      /* Low */
	case 2: return FAN_SPEED_3;      /* Medium */
	case 3: return FAN_SPEED_5;      /* High */
	case 5: return FAN_SPEED_AUTO;   /* Auto */
	default: return FAN_SPEED_AUTO;
	}
}
...
void zcl_callbacks_ready(void)
{
	...
	if (last_sent.temp_c < 16) last_sent.temp_c = 16;
	if (last_sent.temp_c > 30) last_sent.temp_c = 30;
...
static void send_ir_if_changed(void)
{
	...
	if (state.temp_c < 16) state.temp_c = 16;
	if (state.temp_c > 30) state.temp_c = 30;

	/* Skip if unchanged */
	if (last_sent_valid &&
	    state.power == last_sent.power &&
	    state.mode == last_sent.mode &&
	    state.temp_c == last_sent.temp_c &&
	    state.fan == last_sent.fan) {
		return;
	}

	LOG_INF("IR: power=%d mode=%d temp=%dC fan=%d",
		state.power, state.mode, state.temp_c, state.fan);
				       ...
				       uint8_t type, uint16_t size, uint8_t *value)
{
	...
	if (clusterId == Thermostat::Id) {
		ChipLogProgress(Zcl,
				"Thermostat attr 0x%04x changed",
				attributeId);
		send_ir_if_changed();
	} else if (clusterId == FanControl::Id) {
		ChipLogProgress(Zcl,
				"FanControl attr 0x%04x changed",
				attributeId);
		send_ir_if_changed();
	}
}
