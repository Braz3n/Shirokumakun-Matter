...
static AirQuality::Instance air_quality_instance(
	3, /* endpointId */
...
{
	if (co2_ppm < 400)  return AirQuality::AirQualityEnum::kGood;
	if (co2_ppm < 600)  return AirQuality::AirQualityEnum::kFair;
	if (co2_ppm < 1000) return AirQuality::AirQualityEnum::kModerate;
	if (co2_ppm < 1500) return AirQuality::AirQualityEnum::kPoor;
	if (co2_ppm < 2000) return AirQuality::AirQualityEnum::kVeryPoor;
...
static void update_matter_attributes(uint16_t co2_ppm, int16_t temp_001c, uint16_t rh_001pct)
{
	...
	LOG_INF("SCD40: CO2=%u ppm, T=%d.%02d C, RH=%u.%02u %%",
		co2_ppm,
		temp_001c / 100, (temp_001c >= 0 ? temp_001c : -temp_001c) % 100,
		rh_001pct / 100, rh_001pct % 100);
}
